...
function PORM(logDetails, rowIndex) {
    ...
    if (rowIndex > 0) {
        ...
        Focus8WAPI.getFieldValue("fieldValueCallbackIssueBS_PORM", ["", "*","POAbbrList"], Focus8WAPI.ENUMS.MODULE_TYPE.TRANSACTION, false, ++requestId);
    }
...
function fieldValueCallbackIssueBS_PORM(response) {
    ...
    VendorAC = response.data[4].FieldValue;
    Branch = response.data[5].FieldValue;
    Dept = response.data[6].FieldValue;
    Warehouse = response.data[8].FieldValue;
    FGPlanMonth= response.data[10].FieldValue; 
...
function BodyRowdataSplitrollBS_PORM(response) {
    ...
    if (row > 0)  {
        ...
        var postingData = { CompanyId: CID, SessionId: SessID, User: UserName, LoginId: LoginId, irowNo: row, vtype: ivouchertype, docNo: docNo, docDate: docDate, Branch: Branch, iBookNo: VendorAC, Dept: Dept, Warehouse: Warehouse,FGPlanMonth: FGPlanMonth,BodyData: table1 };
        ...
        if (Item_d  > 0 )
        {
            ...
            var url = `/F9ICSKaleGrp_As/PORM/CheckPORM?CompanyId=${CID}&SessionId=${SessID}&User=${UserName}&LoginId=${LoginId}&irowNo=${row}&vtype=${ivouchertype}&docNo=${docNo}&docDate=${docDate}&Branch=${Branch}&iBookNo=${VendorAC}&Dept=${Dept}&Warehouse=${Warehouse}&Item_d=${Item_d}&FGPlanMonth=${FGPlanMonth}&POAbbrList=${POAbbrList}&BodyData=${table1}`;
            table1 = [];
            console.log(url);
