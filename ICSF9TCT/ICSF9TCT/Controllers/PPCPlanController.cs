...
namespace ICSF9TCT.Controllers
{
    ...
    public class PPCPlanController : Controller
    {
        ...
        public string vPAbbr = "PPSfgReqd";
        ...
        public ActionResult UpdateProdnPlan(int CompanyId, string SessionId, string User, int LoginId, int vtype, string docNo, List<PEBody> BodyData)
        {
            ...
            if (PostingStatus == 0)
            {
                ...
                try
                {
                    ...
                    strQry = $@" SELECT CH.iDate, CHD.sNarration, CI.iProduct,CI.fQuantity,CI.iUnit,  CD.iDueDate  " +
                    ...
                    if (ds6 != null && ds6.Tables.Count > 0 && ds6.Tables[0].Rows.Count > 0)
                    {
                        ...
                        bHPPCDueDate = Convert.ToInt32(ds6.Tables[0].Rows[0][5]);

                    }

                    for (int i = 0; i <= ds6.Tables[0].Rows.Count - 1; i++)
                    {
                        @iProductId = Convert.ToInt32(ds6.Tables[0].Rows[i][2]);
                        @pqty = Convert.ToDecimal(ds6.Tables[0].Rows[i][3]);
                    ...
                    clsGeneric.writeLog("URL of PPCPlan :" + "http://localhost/Focus8API/Transactions/Vouchers/" + vPvType);
                    using (var clientPPCPlan = new WebClient())
                    {
                        ...
                        var responsePPCPlan = clientPPCPlan.UploadString("http://localhost/Focus8API/Transactions/Vouchers/"+ vPvType, sContentPPCPlan);
        ...
        public ActionResult UpdateProdnPlanR1(int CompanyId, string SessionId, string User, int LoginId, int vtype, string docNo, List<PEBody> BodyData)
        {
            Sys_IPAddress = Dns.GetHostEntry(Dns.GetHostName()).AddressList.First(x => x.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork).ToString();
                ...
                try
                {
                    ...
                    if (ds2 != null && ds2.Tables.Count > 0 && ds2.Tables[0].Rows.Count > 0)
                    {
                        for (int j = 0; j <= ds2.Tables[0].Rows.Count - 1; j++)
                        {
                            ...
                            strQry = "SELECT TOP 1 mMRP_BOMBody.iInvTagValue FROM mMRP_BOMBody INNER JOIN ICS_BOMVariant ON ICS_BOMVariant.iVariantId =mMRP_BOMBody.iVariantId WHERE mMRP_BOMBody.bMainOutPut=0 AND (ICS_BOMVariant.iVariantId =" + Convert.ToInt32(ds2.Tables[0].Rows[j][1]) + ")";
